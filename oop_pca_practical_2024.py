...
Original file is located at
    https://colab.research.google.com/drive/1tFocpN0JKoNQy3-2ssG9UYiLExM7pjmg
...
def covariance(X):
  ...
  ## Your code here
  cov =(1/(X.shape[0]-1))*np.matmul(X.T,X)
...
# Your code here
eigen_values, eigen_vectors =eig(covariance(X_std))   # return eigen values and eigen vectors
...
class PCA:
    ...
    def transform(self, X):
        X_std = self.Standardize_data(X)
        top_eigen_vectors = self.eigen_vectors[:, :self.n_component]
        transformed_data = np.dot(X_std, top_eigen_vectors)
    ...
    def std(self, X):
        std =((np.sum(((X-self.mean(X))**2),axis=0))/ (X.shape[0]-1))**0.5
        return std
